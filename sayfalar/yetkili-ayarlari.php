                ...
                <tbody>
                    ...
                    if($kullanicilar->rowCount()) {
                      foreach($kullanicilar as $kullaniciveri) {
                        ...
                        <td><a href="#duzenle" onclick="duzenle('; echo "'".htmlspecialchars($kullaniciveri["id"], ENT_QUOTES)."', '".htmlspecialchars($kullaniciveri["kullaniciadi"], ENT_QUOTES)."')".'"'; echo 'style="color:white;" class="btn btn-secondary btn-sm"><i class="fa fa-edit"></i> Düzenle</a></td>
                      </tr>'; 
                      }
                    }
                    ?>
...
$.post("/postV4/kullanici-yasakla", {"kullanici_id":id}, function(cikti) {
  if(cikti.id==1) {
  alert("Yasaklandı!");
  location.reload();
  } else {
