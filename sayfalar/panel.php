...
if($_GET["sayfa"]) { if(!is_numeric($_GET["sayfa"])) { exit; } }

if(empty(trim($_GET["sayfa"]))) { $gosterilecek_sayfa = 1; } else { $gosterilecek_sayfa = $_GET["sayfa"]; }
...
<body class="app sidebar-mini">
    ...
    <header class="app-header">
      ...
      <ul class="app-nav">
        ...
        <li class="app-search">
          <input class="app-search__input" type="search" name="arama" placeholder="Site araması yap" value="<?php if($_GET["arama"]) { echo htmlspecialchars($_GET["arama"], ENT_QUOTES); } ?>" required>
    ...
    <main class="app-content">
      ...
      <div class="col-md-12">
          <div class="tile">
            ...
            <a href="?durum=1" class="btn btn-warning"><i class="fa fa-clock-o"></i> PENDING<?php if($_GET["durum"]==1) { echo " - LISTELENIYOR"; } ?></a><a href="?durum=2" class="btn btn-secondary" style="margin-left:5px;"><i class="fa fa-user-secret"></i> ŞÜPHELİ<?php if($_GET["durum"]==2) { echo " - LISTELENIYOR"; } ?></a><a href="?durum=3" class="btn btn-danger" style="margin-left:5px;"><i class="fa fa-times-circle"></i> FRAUD<?php if($_GET["durum"]==3) { echo " - LISTELENIYOR"; } ?></a> <a href="?durum=4" class="btn btn-success"><i class="fa fa-check"></i> WHITELIST<?php if($_GET["durum"]==4) { echo " - LISTELENIYOR"; } ?></a>
            </div>
                ...
                <tbody>
                    ...
                    if($siteler->rowCount()) {
                      foreach($siteler as $siteveri) {
                        ...
                        <td><a href="#duzenle" onclick="duzenle('; echo "'".htmlspecialchars($siteveri["id"], ENT_QUOTES)."', '".htmlspecialchars($siteveri["site_adresi"], ENT_QUOTES)."')".'"'; echo 'style="color:white;" class="btn btn-secondary btn-sm"><i class="fa fa-edit"></i> Düzenle</a> <a onclick="sil('; echo "'".htmlspecialchars(($siteveri["id"]))."', 1)".'"'; echo 'style="color:white;" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i> Sil</a></td>
                      </tr>';
                      }
                    }
                    ?>
...
$.post("/postV4/site-duzenle", {"site_id":$("#site_id").val(), "durum":$("#durum option:selected").val()}, function(cikti) {
        if(cikti.id==1) {
...
$.post("/postV4/site-sil", {"site_id":id}, function(cikti) {
  if(cikti.id==1) {
  alert("Silindi!");
  $("#"+id+"_site").remove();
  if(akabin==2) {
